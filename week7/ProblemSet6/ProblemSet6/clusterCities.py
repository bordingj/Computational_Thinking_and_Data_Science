...
class Cluster(object):
    ...
    def singleLinkageDist(self, other):
        ...
        for i in xrange(len(self.points)):
                current_self_member = next(self_members)
                other_members = other.members()
                for i in xrange(len(other.points)):
                    current_other_member = next(other_members)
                    distances_list.append(current_other_member.distance(current_self_member))
    ...
    def averageLinkageDist(self, other):
        ...
        for i in xrange(len(self.points)):
                ...
                for i in xrange(len(other.points)):
                    current_other_member = next(other_members)
                    distances_list.append(current_other_member.distance(current_self_member))
        return sum(distances_list)/float(len(distances_list))
...
class ClusterSet(object):
    ...
    def mergeClusters(self, c1, c2):
        ...
        merged_Cluster = []  
        c1_members = c1.members()
        for i in xrange(len(c1.points)):
            merged_Cluster.append(next(c1_members))
        c2_members = c2.members()
        for i in xrange(len(c2.points)):
            merged_Cluster.append(next(c2_members))
        merged_Cluster = Cluster(merged_Cluster, c1.pointType)
       ...
       # def comb2(s):
        #    for i, v1 in enumerate(s):
         #       for j in range(i+1, len(s)):
          #          yield (v1, s[j])
        comb = list(itertools.combinations(self.getClusters(),2))
        
        min_dist = linkage(comb[0][0],comb[0][1])
        closests = comb[0]
    ...
    def mergeOne(self, linkage):
        ...
        # TO DO
        closests = self.findClosest(linkage)
    #    import copy
    #    copied_closests = copy.copy(closests)
        self.mergeClusters(closests[0],closests[1])
        return closests        
