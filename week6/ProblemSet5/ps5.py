...
print g
"""
"""each node will represent a building and each edge represent how you can get from one building to another"""
...
def load_map(mapFilename):
    ...
    for edge in lines:
        try:
            source = g.addNode(str(edge[0]))
            dist = g.addNode(str(edge[1]))
            g.addEdge(WeightedEdge(source, dist, float(edge[2]), float(edge[3])))
        except:
            try:
                source = g.addNode(str(edge[0]))
                g.addEdge(WeightedEdge(source, str(edge[1]), float(edge[2]), float(edge[3])))
            except:
                try:
                    dist = g.addNode(str(edge[1]))
                    g.addEdge(WeightedEdge(str(edge[0]), dist, float(edge[2]), float(edge[3])))
                except:
                    g.addEdge(WeightedEdge(str(edge[0]), str(edge[1]), float(edge[2]), float(edge[3])))
